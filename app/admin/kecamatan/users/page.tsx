    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 font-medium mt-1">Kelola akun operator sekolah dan admin.</p>
        </div>
        <Button
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="pb-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">User</th>
                  <th className="pb-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Role</th>
                  <th className="pb-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">NPSN</th>
                  <th className="pb-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Dibuat</th>
              ...
              <tbody className="divide-y divide-white/5">
                {loading ? (
                  Array(5).fill(0).map((_, i) => (
                    <tr key={i}>
                      <td className="py-4"><Skeleton className="h-10 w-40 rounded-xl bg-white/5" /></td>
                      <td className="py-4"><Skeleton className="h-6 w-20 rounded-full bg-white/5" /></td>
                      <td className="py-4"><Skeleton className="h-4 w-16 bg-white/5" /></td>
                      <td className="py-4"><Skeleton className="h-4 w-24 bg-white/5" /></td>
                    ...
                    <tr key={u.id} className="group hover:bg-white/5 transition-colors">
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center font-black text-slate-300 uppercase">
                            {u.full_name?.charAt(0) || '?'}
                          </div>
                          <div>
                            <p className="font-bold text-white leading-tight">{u.full_name || 'No Name'}</p>
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">ID: {u.id.slice(0, 8)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4">
                        <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${u.role === 'admin_kecamatan' ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/20' : 'bg-white/5 text-slate-400 border border-white/5'
                          }`}>
                          {u.role.replace('_', ' ')}
                        </span>
                      </td>
                      <td className="py-4 font-bold text-slate-300 tracking-tight">{u.npsn || '-'}</td>
                      <td className="py-4 text-xs font-bold text-slate-500">
                        {new Date(u.created_at).toLocaleDateString('id-ID')}
                      </td>
                      <td className="py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white" onClick={() => handleOpenModal(u)}>
                            <Key className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-lg hover:bg-red-500/10 text-red-400 hover:text-red-500" onClick={() => handleDeleteUser(u.id)}>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-white/10 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-white tracking-tight">{editingUser ? 'Edit User' : 'Tambah User Baru'}</h2>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {['operator', 'admin_kecamatan'].map((r) => (
                        <button
                          ...
                          className={`p-4 rounded-2xl border transition-all text-left group ${formData.role === r ? 'border-blue-500 bg-blue-500/10' : 'border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20'
                            }`}
                        >
                          <div className={`w-8 h-8 rounded-lg mb-3 flex items-center justify-center transition-colors ${formData.role === r ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white/10 text-slate-400 group-hover:text-white'
                            }`}>
                            {r === 'admin_kecamatan' ? <Shield className="h-4 w-4" /> : <School className="h-4 w-4" />}
                          </div>
                          <p className={`font-black uppercase tracking-widest text-[10px] ${formData.role === r ? 'text-blue-400' : 'text-slate-400'
                            }`}>{r.replace('_', ' ')}</p>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        className="h-12 w-12 rounded-xl p-0 border-white/10 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white"
                      >
                        <RefreshCw className="h-5 w-5" />
                      </Button>
                    </div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Gunakan password yang kuat</p>
                  </div>
                </div>
